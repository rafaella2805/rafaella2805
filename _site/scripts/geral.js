...
$(document).ready(function() { //when the document is ready...			
	...
	$(".bg").everyTime(6, function(){						 
		$(".bg").animate({bottom:"300px"}, 900).animate({bottom:"-30px"}, 900);
  	});
	...
	$("#home, #about, #portfolio, #contato, #lab, #about, #jobs").bind("inview", function (event, visible) {
			if (visible) {
	...
	*/
	...
	function Move(){ 
		...
		if($homeBG.hasClass("inview")){
			...
			homeanimate.css({'backgroundPosition': newPos(0, windowHeight, pos, 1143, 0.6)});
		}
		...
		if($portfolio.hasClass("inview")){
			...
			//call the newPos function and change the secnond background position
			portfolioanimate.css({'backgroundPosition': newPos(100, windowHeight, pos, 3640, 0.6)});
		}
